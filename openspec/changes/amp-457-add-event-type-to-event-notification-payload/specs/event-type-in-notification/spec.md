...
#### Scenario: eventType is null when source event has no eventType
- **WHEN** `NotificationMapper.mapToPayload()` is called with an `EventPayload` where `eventType` is null
- **THEN** the returned `EventNotificationPayload` SHALL have a null `eventType`
