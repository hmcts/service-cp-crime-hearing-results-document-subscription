...
public class NotificationController implements InternalApi, NotificationApi {
    ...
    public ResponseEntity<Void> createNotification(
        ...
        log.info("Received notification request from Progression/HearingNows Service - eventId: {}, hearingId: {}, materialId: {}, eventType: {}",
                eventPayload.getEventId(),
                eventPayload.getHearingId(),
                eventPayload.getMaterialId(),
                eventPayload.getEventType());

        if (appProperties.isHearingEventDisabledInDev() && appProperties.getEnvironmentName() == EnvironmentName.DEV) {
            log.warn("Notification processing disabled in DEV swallowing eventId:{}", eventPayload.getEventId());
    ...
    @Override
    @AuditDetail(eventName = "hrds.get-document", action = "View", pathParams = {"clientSubscriptionId", "documentId"})
